...
Begin
    ...
    BEGIN
      EXECUTE IMMEDIATE 'ALTER TABLE einwohner ADD CONSTRAINT pk_einwohner PRIMARY KEY (id)';
    EXCEPTION
        WHEN OTHERS THEN
          dbms_output.put_line('PRIMARY KEY existiert bereits');
    END;

    BEGIN
      EXECUTE IMMEDIATE 'CREATE BITMAP INDEX bitmap_vater_mutter ON einwohner (vaterid, mutterid, 1)';
    EXCEPTION
        WHEN OTHERS THEN
          dbms_output.put_line('CREATE BITMAP INDEX bitmap_vater_mutter existiert bereits');
    END;

    BEGIN
      EXECUTE IMMEDIATE 'REATE MATERIALIZED VIEW vater_mutter_not_null
      BUILD IMMEDIATE
    ...
    EXCEPTION
        WHEN OTHERS THEN
          dbms_output.put_line('MATERIALIZED VIEW vater_mutter_not_null existiert bereits');
      END;

    BEGIN
      EXECUTE IMMEDIATE 'CREATE INDEX idx_month ON einwohner (extract(month from geburtsdatum)';
    EXCEPTION
      WHEN OTHERS THEN
        dbms_output.put_line('CREATE INDEX idx_month existiert bereits');
    END;
